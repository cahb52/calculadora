var calculadora = {
	...
	EventosFormatoBotones: function(selector){
		...
		for (var i = 0; i<x.length;i++) {
			x[i].onmouseover = this.ReducirBoton;
			x[i].onmouseleave = this.eventoVuelveBoton;
		};
	},
	ReducirBoton: function(event){
	...
	ingresoOperacion: function(oper){
		this.valorInicial = parseFloat(this.valorVisor);
		this.valorVisor = "";
		this.operacion = oper;
		this.BotonIgual = false;
		this.updateVisor();
	...
	verResultado: function(){
		...
		if(!this.BotonIgual){
			...
			this.realizarOperacion(this.valorInicial, this.segundoValor, this.operacion);

		} else {
			this.realizarOperacion(this.valorInicial, this.ultimoValor, this.operacion);
		}

		this.valorInicial = this.resultado;
		this.valorVisor = "";
	...
	realizarOperacion: function(valorInicial, segundoValor, operacion){
		switch(operacion){
			case "+":
				this.resultado = eval(valorInicial + segundoValor);
			break;
			case "-":
				this.resultado = eval(valorInicial - segundoValor);
			break;
			...
			break;
			case "raiz":
				this.resultado = eval(Math.sqrt(valorInicial));
		}
	},
